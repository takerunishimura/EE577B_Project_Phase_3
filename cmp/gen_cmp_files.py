...
XY routing: route X first (East/West), then Y (North/South)
  dirX=0 (East):  dest_col > src_col
  dirX=1 (West):  dest_col < src_col
  dirY=0 (South): dest_row > src_row  (row increases downward)
  dirY=1 (North): dest_row < src_row
...
def build_packet(src_row, src_col, dst_row, dst_col):
    """
    Build a 64-bit packet using XY routing.
    Route X (col) first, then Y (row).
    
    vc: alternate even/odd based on (abs(dx)+abs(dy)) parity
        use vc=0 (even) for simplicity — all packets use even VC
    dirX: 0=East(dst_col>src_col), 1=West(dst_col<src_col) <- THIS IS WRONG, 1=EAST AND 0=WEST
    dirY: 0=South(dst_row>src_row), 1=North(dst_row<src_row) 
    hopX: unary encoding of abs(dst_col - src_col)
    hopY: unary encoding of abs(dst_row - src_row)
    sourceX: src_col (8-bit)
    sourceY: src_row (8-bit)
    payload: upper 16 bits = source node id, lower 16 bits = dest node id
    """
    dx = dst_col - src_col
    dy = dst_row - src_row

    # VC: use even (0) for all packets to keep it simple
    vc = 0

    # Direction X - ROUTER convention: 1=East, 0=West
    dirX = 1 if dx > 0 else 0   # 1=East if going right

    # Direction Y - ROUTER convention: 1=North, 0=South  
    dirY = 1 if dy < 0 else 0   # 1=North if going up (smaller row)

    # Hop counts (unary encoded, 4 bits each)
    hopX = unary_hop(abs(dx)) & 0xF
    hopY = unary_hop(abs(dy)) & 0xF

    # Source coordinates
    sourceX = src_col & 0xFF
    sourceY = src_row & 0xFF

    # Payload: encode src and dst node IDs
    src_id = src_row * 4 + src_col
    dst_id = dst_row * 4 + dst_col
    payload = ((src_id & 0xFF) << 8) | (dst_id & 0xFF)  # 16-bit payload

    # Assemble 64-bit packet (big-endian bit ordering)
    # bits[0:31] = header, bits[32:63] = payload
    header = 0
    header |= (vc    & 0x1) << 31    # bit[0]  = MSB of 32-bit header
    header |= (dirX  & 0x1) << 30    # bit[1]
    header |= (dirY  & 0x1) << 29    # bit[2]
    # bits[3:7] = reserved = 0 (bits 28:24 of header)
    header |= (hopX  & 0xF) << 20    # bits[8:11]  (bits 23:20)
    header |= (hopY  & 0xF) << 16    # bits[12:15] (bits 19:16)
    header |= (sourceX & 0xFF) << 8  # bits[16:23] (bits 15:8)
    header |= (sourceY & 0xFF) << 0  # bits[24:31] (bits 7:0)
...
def main():
    ...
    for row in range(4):
        for col in range(4):
            ...
            fname = f"testcase/cmp_test.imem.{nn}.fill"
    ...
    for row in range(4):
        for col in range(4):
            ...
            fname = f"testcase/cmp_test.dmem.{nn}.fill"
    ...
    print("\nDone! All files written to testcase/")
